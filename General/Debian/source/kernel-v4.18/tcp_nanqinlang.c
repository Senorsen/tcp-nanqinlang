...
struct bbr {
	...
	u32     mode:3,		     /* current bbr_mode in state machine */
		...
		min_tso_segs:7,     /* segments we want in each skb we send */
...
static void bbr_tso_segs_goal(struct sock *sk)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct bbr *bbr = inet_csk_ca(sk);
	u32 min_segs;
	...
	bbr->min_tso_segs = min(tcp_tso_autosize(sk, tp->mss_cache, min_segs),
				 0x7FU);
}
 ...
 * in such cases min_tso_segs is 1. The minimum cwnd is 4 packets,
...
static void bbr_init(struct sock *sk)
{
	...
	bbr->prior_cwnd = 0;
	bbr->min_tso_segs = 0;	 /* default segs per skb until first ACK */
	bbr->rtt_cnt = 0;
